...
2. 여러 군데를 동시에 클릭 가능합니다.
3. 마스크가 만족스러우면 **확인 (마스크 확정)** 을 클릭합니다.
4. 마우스 우클릭을 통해 선택된 영역 중 일부를 제거할 수 있습니다.
5. 초기화를 통해 다시 처음부터 선택 가능합니다.
6. **Step 3: Inpainting으로 전달** 버튼을 클릭합니다.

**Step 3 — Inpainting (탭 ③)**
1. 채울 배경을 설명하는 프롬프트를 입력합니다.
2. 스텝 수와 가이던스를 원하는 만큼 조절합니다(스텝 값이 클수록 정교하나 생성에 오랜시간이 걸립니다).
3. **인페인팅 실행** 버튼을 클릭합니다.
