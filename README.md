...
```
.
├── demo.py            # 통합 파이프라인 진입점 & Gradio UI 빌더
...
> ⚠️ Inpainting은 Segmentation이 만든 마스크를 필수 입력으로 사용하므로 **단독 실행을 지원하지 않습니다.** 반드시 `demo.py`의 통합 파이프라인에서 사용하세요.
...
### ③ inpainting.py
> **단독 실행 불가** — Segmentation 마스크를 필수 입력으로 사용하므로 `demo.py` 파이프라인에서만 동작합니다.
